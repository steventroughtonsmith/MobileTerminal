...
- (id) initWithFileHandle:(NSFileHandle*)fileHandle
{
	...
	if (self != nil) {
		...
		//if (isatty([handle fileDescriptor]))
		{
			if (ioctl([fileHandle fileDescriptor], TIOCGWINSZ, &window_size) == -1) {
...
- (void)setWidth:(int)terminalWidth withHeight:(int)terminalHeight;
{
	...
	if (ioctl([self->handle fileDescriptor], TIOCSWINSZ, &window_size) == -1) {
		[NSException raise:@"IOException"
